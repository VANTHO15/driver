...
volatile boolean frameLED = false;
volatile boolean buttonLED = false;
...
void compute() {
  ...
  if (sensorVal == HIGH) {
    ...
    buttonLED = true;
    framelist[1].data[3] = 0xA0;
  }

  // third, make a decision on the LED to light
  if ((!buttonLED) && (!frameLED)) {
    digitalWrite(13, LOW);
  } else {
